...
class NewUserDialog(QtWidgets.QDialog):
    ...
    def __init__(self, user_object, *args, **kwargs):
        ...
        self._user_password_le = ValidatedString(
        ...
        self._user_password2_le = ValidatedString(
            name="user_password", allow_special_characters=True
        )
        ...
        self._user_name_le.set_connected_widgets(
            self.button_box.button(QtWidgets.QDialogButtonBox.Ok)
        )
        self._initials_le.set_connected_widgets(
            self.button_box.button(QtWidgets.QDialogButtonBox.Ok)
        )
        self._user_password_le.set_connected_widgets(
            self.button_box.button(QtWidgets.QDialogButtonBox.Ok)
        )
        self._user_password2_le.set_connected_widgets(
            self.button_box.button(QtWidgets.QDialogButtonBox.Ok)
        )
...
class LoginDialog(QtWidgets.QDialog):
    ...
    def on_accept(self):
        ...
        state, msg = self.user_object.set(
            _user, _password, save_to_db=_remember, clear_db=not _remember
        )
        if state != -1:
            self.accept()
        else:
